...
export class TournamentService {
    ...
    getTournaments(): void {
        this.http.get<Tournament[]>(RESOURCE_URL).subscribe((tournaments: Tournament[]) => {
           this.tournamentService.next({
             entities: tournaments.reduce((acc, tournament) => ({ ...acc, [tournament.id]: tournament }), {}),
    ...
    getTournamentById(id: number): void {
        this.http.get<Tournament>(`${RESOURCE_URL}/${id}`).subscribe((tournament: Tournament) => {
    ...
    createTournament(tournament: UnknownTournament): void {
        this.http.post<Tournament>(RESOURCE_URL, tournament)
    ...
    enrollPlayer(tournamentId: number, user: User): void {
        this.http.post<Tournament>(`${RESOURCE_URL}/${tournamentId}/players`, user)
            .subscribe((resTournament: Tournament) => {
                const { entities, orderedIds } = this.tournamentService.value;
                this.tournamentService.next({
                    entities: { ...entities, [resTournament.id]: resTournament },
    ...
    withdrawPlayer(tournamentId: number, playerId: number): void {
        this.http.delete<Tournament>(`${RESOURCE_URL}/${tournamentId}/players/${playerId}`)
            .subscribe((resTournament: Tournament) => {
                const { entities, orderedIds } = this.tournamentService.value;
                this.tournamentService.next({
                    entities: { ...entities, [resTournament.id]: resTournament },
    ...
    rankPlayer(tournamentId: number, ranking: Ranking): void {
        this.http.put<Tournament>(`${RESOURCE_URL}/${tournamentId}/rankings`, ranking)
            .subscribe((resTournament: Tournament) => {
                const { entities, orderedIds } = this.tournamentService.value;
                this.tournamentService.next({
                    entities: { ...entities, [resTournament.id]: resTournament },
        ...
        this.http.delete(`${RESOURCE_URL}/${id}`).subscribe(() => {
           ...
           this.tournamentService.next({
              entities: Object.values(entities)
                  ...
                  .reduce((acc, tournament: Tournament) => ({ ...acc, [tournament.id]: tournament}), {}),
